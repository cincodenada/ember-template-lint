...
module.exports = class TodoHandler {
  ...
  update(results, writeTodoOptions) {
    ...
    let todoBatchCounts = writeTodos(this._workingDir, this._processedResults, writeTodoOptions);
    ...
    for (const todo of [...stable.values()]) {
      ...
      let result = results.find(
        (result) =>
          ...
          result.line === todo.range.start.line &&
          result.column === todo.range.start.column
      );
